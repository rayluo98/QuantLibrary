...
void nbpoolinit(nbpool* pool, ae_int_t n, ae_state *_state)
{
    ...
    pool->n = n;
    threadunsafeset(&pool->temporariescount, 0, _state);
    if( n==0 )
    {
        return;
    }
     ...
     ae_state *_state)
{
    ...
    if( threadunsafeget(&pool->temporariescount, _state)>apserv_maxtemporariesinnpool )
    {
        threadunsafeset(&pool->temporariescount, 0, _state);
     ...
     ae_state *_state)
{
    ...
    if( threadunsafeget(&pool->temporariescount, _state)<0 )
    {
        threadunsafeset(&pool->temporariescount, 0, _state);
    }
    ae_frame_leave(_state);
